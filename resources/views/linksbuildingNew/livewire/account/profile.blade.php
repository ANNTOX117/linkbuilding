<div class="container-fluid">
    <div class="support-list bg-white p-3 shadow">
    	<div class="table-title d-flex flex-column flex-lg-row justify-content-between p-3">
			<h4 class="m-0 p-0 my-4 text-center text-lg-left font-weight-bold">{{__('My profile settings')}}</h4>
		</div>
		...
		<div class="white-container my-4">
			<div class="row justify-content-center justify-content-lg-between">
				<div class="col-6 col-lg-3">
					<div class="image-upload d-flex flex-column">
						...
						<input type="file" id="file" wire:model="profile_image" wire:change="$emit('fileChoosen')" :errors="$errors"  style="display:none"/>
						@if($image && !$uploaded)
							<a class="text-primary btn-save-image-profile" style="cursor: pointer;" wire:click="uploadImage">{{__('Save')}}</a>
						@endif
					</div>
				</div>
			...
			<div class="w-100 d-flex justify-content-center justify-content-lg-end my-4">
				<ul class="nav nav-tabs border-0" id="myTab" role="tablist">
					<li class="nav-item">
						<a class="nav-link px-4 @if($tab == "Personal") active @endif" wire:click="changeTab('Personal')" data-toggle="tab" href="#startpage-link">{{__('Personal details')}}</a>
					</li>
					<li class="nav-item">
						<a class="nav-link px-4 @if($tab == "Login") active @endif" wire:click="changeTab('Login')" data-toggle="tab" href="#blog-sidebar-link">{{__('Login details')}}</a>
					</li>
                    @if($is_company)
                        <li class="nav-item">
                            <a class="nav-link px-4 @if($tab == "Company") active @endif" wire:click="changeTab('Company')" data-toggle="tab" href="#company">{{__('Company information')}}</a>
                        </li>
                    @endif
				</ul>
			</div>
