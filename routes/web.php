...
Route::get('xmlSiteMap/{segment}.xml', [App\Http\Controllers\CreateSitemapController::class, 'index'])->name('xmlSiteMap');
...
$domain   = domain();
$category = subdomain();
//$domain = "https://www.hotpaginas.nl";
$category = '';
...
if(!empty($website)) 
{
    $template = Template::getTemplateName($website->id);
    try {
        switch($template->slug){
            case "datingtemplateprofile1":
                include 'webDatingTemplateProfile1.php';
            break;
        }
    } catch (\Throwable $th) {
       include 'webDefaultTemplate.php';
    }
} 
else 
{
    ...
    Route::group(['prefix' => LaravelLocalization::setLocale(), 'middleware' => ['web', 'auth:sanctum', 'verified', 'admin', 'xss'], 'namespace' => '\App\Http\Livewire\Admin'], function ($router) {
        ...
        Route::get('admin/seo-pages', SeoPages::class)->name('seo-pages');
        Route::get('admin/templates', Templates::class)->name('templates');
        Route::get('admin/banners', Banners::class)->name('banners');
